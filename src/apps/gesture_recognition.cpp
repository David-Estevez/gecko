...
int main (int argc, char * argv[])
{
	...
	switch (argc)
	{
		...
		case 0: default: 
				std::cerr<<"Usage: ./[executable_name] path_to_picture1 [optional: path_to_picture2]"<<std::endl; 
				return 1; 
				
	}	


	matchTemplate (frame,templ, result,0);
	cv::normalize(result, result, 1, 0, CV_MINMAX);
	...
	cv::rectangle( frame, matchLoc, cv::Point( matchLoc.x + templ.cols , matchLoc.y + templ.rows ), cv::Scalar::all(0), 2, 8, 0 );
